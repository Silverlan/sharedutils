...
#ifndef __UTIL_HANDLE_HPP__
#define __UTIL_HANDLE_HPP__
...
template<typename T>
util::THandle<T>::THandle() : m_data {nullptr}
{
}

template<typename T>
util::THandle<T>::THandle(std::nullptr_t) : m_data {nullptr}
{
}

template<typename T>
util::THandle<T>::THandle(const std::shared_ptr<PtrHandleData> &data) : m_data {data}
{
}
template<typename T>
bool util::THandle<T>::operator==(std::nullptr_t) const
{
...
template<typename T>
bool util::THandle<T>::operator==(const THandle<T> &other) const {
	if (!m_data)
		return !other.m_data;
	return m_data->GetData() == other.m_data->GetData();
}
template<typename T>
bool util::THandle<T>::operator==(const T *other) const {
	if (!m_data)
		return !other;
	return m_data->GetData() == other;
}
template<typename T>
T *util::THandle<T>::operator->()
{
	return Get();
}
template<typename T>
const T *util::THandle<T>::operator->() const
{
	return const_cast<util::THandle<T> *>(this)->operator->();
}
template<typename T>
T &util::THandle<T>::operator*()
{
...
template<typename T>
bool util::THandle<T>::IsValid() const
{
	return m_data && m_data->GetData() != nullptr;
}
template<typename T>
bool util::THandle<T>::IsExpired() const
{
	return IsValid() == false;
}
template<typename T>
const T *util::THandle<T>::Get() const
{
	return const_cast<THandle<T> *>(this)->Get();
}
template<typename T>
T *util::THandle<T>::Get()
{
	return IsValid() ? static_cast<T*>(m_data->GetData()) : nullptr;
}
template<typename T>
void util::THandle<T>::Remove()
{
	m_data->Invalidate();
}

template<typename T>
const std::shared_ptr<util::PtrHandleData> &util::THandle<T>::GetInternalData() const
{
	return m_data;
}
...
template<class TDst>
util::THandle<TDst> util::create_handle(TDst *ptr)
{
	return util::THandle<TDst> {std::make_shared<PtrHandleData>(ptr)};
}
