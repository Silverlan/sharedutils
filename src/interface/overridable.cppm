...
export namespace util {
	...
	namespace _impl {
		template<typename T>
		class OverridableImpl : public std::enable_shared_from_this<OverridableImpl<T>> {
		  public:
		  ...
		  protected:
			static std::shared_ptr<OverridableImpl<T>> Create() { return std::shared_ptr<OverridableImpl<T>>(new OverridableImpl<T>()); }
			static std::shared_ptr<OverridableImpl<T>> Create(const T &v) { return std::shared_ptr<OverridableImpl<T>>(new OverridableImpl<T>(v)); }
	...
	class Overridable {
	  public:
		...
		CallbackHandle AddChangeCallback(const std::function<void(std::reference_wrapper<const T>, std::reference_wrapper<const T>)> &f) { return m_impl->AddChangeCallback(f); }
