...
export {
	namespace util {
		...
		template<class TDataStreamBase>
		template<class T>
		DataStreamBase &TDataStream<TDataStreamBase>::operator<<(T *v)
		{
			return (*this->get()) << v;
		}
		template<class TDataStreamBase>
		template<class T>
		DataStreamBase &TDataStream<TDataStreamBase>::operator<<(const T &v)
		{
			return (*this->get()) << v;
		}
		template<class TDataStreamBase>
		...
		template<class TDataStreamBase>
		TDataStream<TDataStreamBase>::TDataStream(const TDataStream &o) : m_baseStreamObject {o.m_baseStreamObject}
		{
		}
		template<class TDataStreamBase>
		TDataStream<TDataStreamBase>::TDataStream() : m_baseStreamObject {new TDataStreamBase}
		{
			(*this)->Resize(1);
		}
		template<class TDataStreamBase>
		TDataStream<TDataStreamBase>::TDataStream(uint32_t size) : m_baseStreamObject {new TDataStreamBase(size)}
		{
			(*this)->Resize(size);
		}
		template<class TDataStreamBase>
		TDataStream<TDataStreamBase>::TDataStream(void *data, uint32_t size) : m_baseStreamObject {new TDataStreamBase(data, size)}
		{
		}
		template<class TDataStreamBase>
		TDataStream<TDataStreamBase>::TDataStream(std::nullptr_t) : m_baseStreamObject {nullptr}
		{
		}
		template<class TDataStreamBase>
		bool TDataStream<TDataStreamBase>::IsValid() const
		{
			return (this->get() == nullptr) ? false : true;
		}
		...
		template<class TDataStreamBase>
		bool TDataStream<TDataStreamBase>::operator==(const TDataStream &p) const
		{
			return (this->get() == p.get()) ? true : false;
		}
		template<class TDataStreamBase>
		bool TDataStream<TDataStreamBase>::operator!=(const TDataStream &p) const
		{
		...
		template<class TDataStreamBase>
		bool TDataStream<TDataStreamBase>::operator==(const std::nullptr_t) const
		{
		...
		template<class TDataStreamBase>
		DataStreamBase &TDataStream<TDataStreamBase>::operator<<(const std::string &str)
		{
			return (*this->get()) << str;
		}
		template<class TDataStreamBase>
		DataStreamBase &TDataStream<TDataStreamBase>::operator<<(const char *str)
		{
			return (*this->get()) << str;
		}
		template<class TDataStreamBase>
		DataStreamBase &TDataStream<TDataStreamBase>::operator>>(std::string &str)
		{
			return (*this->get()) >> str;
		}
...
#pragma warning(pop)
		...
		template<class T>
		DataStreamBase &DataStreamBase::operator<<(T *v)
		{
			Write(v);
			return *this;
		}
		template<class T>
		DataStreamBase &DataStreamBase::operator<<(const T &v)
		{
			Write(v);
			return *this;
		}
