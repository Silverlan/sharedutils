...
export namespace pragma::util {
	...
	class DLLSHUTIL FileAssetManager : public IAssetManager {
	  public:
		static constexpr AssetLoadJobPriority DEFAULT_PRIORITY = 0;
		static constexpr AssetLoadJobPriority IMMEDIATE_PRIORITY = 10;
		...
		struct DLLSHUTIL Callbacks {
			std::function<void(Asset *, AssetLoadResult)> onAssetLoaded;
		...
		virtual void SetLogHandler(const LogHandler &logHandler) override;
		...
		AssetObject LoadAsset(const std::string &path, std::unique_ptr<AssetLoadInfo> &&loadInfo = nullptr, PreloadResult *optOutResult = nullptr);
		AssetObject LoadAsset(const std::string &cacheName, std::unique_ptr<ufile::IFile> &&file, const std::string &ext, std::unique_ptr<AssetLoadInfo> &&loadInfo = nullptr, PreloadResult *optOutResult = nullptr);
		...
		virtual AssetObject ReloadAsset(const std::string &path, std::unique_ptr<AssetLoadInfo> &&loadInfo = nullptr, PreloadResult *optOutResult = nullptr);
		...
		const Path &GetRootDirectory() const;
		void SetImportDirectory(const std::string &dir);
		const Path &GetImportDirectory() const;
		...
		void CallOnLoad(const std::string &path, const std::function<void(Asset *, AssetLoadResult)> &onLoad);
	  ...
	  protected:
		...
		virtual void InitializeProcessor(IAssetProcessor &processor) = 0;
		virtual AssetObject InitializeAsset(const Asset &asset, const AssetLoadJob &job) = 0;
		...
		void RegisterFormatHandler(const std::string &ext, const std::function<std::unique_ptr<IAssetFormatHandler>(IAssetManager &)> &factory, AssetFormatType formatType = AssetFormatType::Binary);
		template<class T>
		void RegisterFormatHandler(const std::string &ext, AssetFormatType formatType = AssetFormatType::Binary)
		{
			return RegisterFormatHandler(ext, [](IAssetManager &assetManager) -> std::unique_ptr<IAssetFormatHandler> { return std::make_unique<T>(assetManager); }, formatType);
		}

		PreloadResult PreloadAsset(const std::string &path, AssetLoadJobPriority priority, std::unique_ptr<AssetLoadInfo> &&loadInfo);
		PreloadResult PreloadAsset(const std::string &cacheName, std::unique_ptr<ufile::IFile> &&file, const std::string &ext, AssetLoadJobPriority priority, std::unique_ptr<AssetLoadInfo> &&loadInfo);
		AssetObject LoadAsset(const std::string &path, const PreloadResult &r);
		AssetObject Poll(std::optional<AssetLoadJobId> untilJob, AssetLoaderWaitMode wait);

		void RegisterImportHandler(const std::string &ext, const std::function<std::unique_ptr<IImportAssetFormatHandler>(IAssetManager &assetManager)> &factory, AssetFormatType formatType = AssetFormatType::Binary);
		template<class T>
		void RegisterImportHandler(const std::string &ext, AssetFormatType formatType = AssetFormatType::Binary)
		{
			return RegisterImportHandler(ext, [](IAssetManager &assetManager) -> std::unique_ptr<IImportAssetFormatHandler> { return std::make_unique<T>(assetManager); }, formatType);
		}
	  ...
	  private:
		void CacheResult(size_t hash, const FileAssetManager::PreloadResult &result);
		void CacheResult(const std::string &assetName, const FileAssetManager::PreloadResult &result);
		std::optional<FileAssetManager::PreloadResult> GetCachedResult(size_t hash) const;
		std::function<std::optional<std::string>(const std::string &, const std::string &)> m_externalSourceFileImportHandler = nullptr;
		std::unordered_map<size_t, std::vector<std::function<void(Asset *, AssetLoadResult)>>> m_callOnLoad;
		std::unique_ptr<AssetFileHandler> m_fileHandler;
		Path m_rootDir;
		Path m_importRootDir;
		std::mutex m_preloadMutex;

		std::unordered_map<size_t, FileAssetManager::PreloadResult> m_errorCache;
		mutable std::mutex m_errorCacheMutex;

		std::unordered_map<std::string, std::function<std::unique_ptr<IImportAssetFormatHandler>(IAssetManager &)>> m_importHandlers;
	};

	template<typename TAssetType, typename TLoadInfo>
	class TFileAssetManager : public FileAssetManager {
	  public:
		...
		std::shared_ptr<TAssetType> LoadAsset(const std::string &path, AssetLoadFlags flags, PreloadResult *optOutResult = nullptr)
		{
	  ...
	  private:
		FileAssetManager::PreloadResult PreloadAsset(const std::string &path, AssetLoadJobPriority priority, std::unique_ptr<TLoadInfo> &&loadInfo) { return FileAssetManager::PreloadAsset(path, priority, std::move(loadInfo)); }
		FileAssetManager::PreloadResult PreloadAsset(const std::string &cacheName, std::unique_ptr<ufile::IFile> &&file, const std::string &ext, AssetLoadJobPriority priority, std::unique_ptr<TLoadInfo> &&loadInfo)
		{
